---
name: own-this
description: Guides a developer through a comprehension interview about their codebase, then generates modular, gap-aware documentation that helps them truly own it. Use when the developer says "own-this", "own this project", "help me understand this codebase", "debrief this project", or asks for help understanding a project. Run from inside the project root.
---

# own-this

Take ownership of any codebase. Scan → interview → gap analysis → docs.
...
organised by **category**: tech stack, packages & dependencies, architecture &
patterns, data flow, non-trivial implementations (WASM, media, auth, real-time,
complex async), config & deployment, risky spots. Prioritize by complexity and
risk. Never show the checklist to the user.

See [REFERENCE.md](REFERENCE.md#stage-1--scan) for detailed scan guidelines.

### 2. Interview (conversational)
Interview the user about their understanding of each category on the checklist.
Conversational tone — not an exam. Lead with **4-option familiarity questions** (a/b/c/d) for every category —
fast, low-friction. (d) is "Let's discuss this" — if picked, the user wants to
talk, so engage conversationally. Track internally:
✅ understood / ⚠️ partial / ❌ gap / ⏭ skipped.
